...
STDEVS = {
    ...
    'att': {'fgsm': 0.133, 'bim-a': 0.0155, 'bim-b': 0.095, 'cw-l2': 0.008}
}
...
def get_noisy_samples(X_test, X_test_adv, attack):
    ...
    if attack in ['jsma', 'cw-l0']:
        ...
        X_test_noisy = np.minimum(
            np.maximum(
                X_test + np.random.normal(loc=0, scale=STDEVS['att'][attack],
                                          size=X_test.shape),
                CLIP_MIN
            ),
            CLIP_MAX
...
def get_layer_wise_activations(model, dataset):
    ...
    :return: 
    """
    assert dataset in ['mnist', 'cifar', 'svhn', 'att'], \
