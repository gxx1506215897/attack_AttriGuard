...
ATTACK_PARAMS = {
    ...
    'att': {'eps': 0.130, 'eps_iter': 0.010, 'image_size': 100, 'num_channels': 1, 'num_labels': 25}
}
...
def craft_one_type(sess, model, X, Y, dataset, attack, batch_size):
    ...
    if attack == 'fgsm':
        ...
        cw_attack = CarliniL2(sess, model, num_channels, num_labels, batch_size=batch_size)
        ...
        num_channels = ATTACK_PARAMS[dataset]['num_channels']
        num_labels = ATTACK_PARAMS[dataset]['num_labels']
        cw_attack = CarliniLID(sess, model, num_channels, num_labels, batch_size=batch_size)
        X_adv = cw_attack.attack(X, Y)
    # X_adv = X_adv.reshape(-1,10000)
...
def main(args):
    ...
    model_file = os.path.join(PATH_DATA, "model_%s.h5" % ('att'))
    ...
    print('Dataset: %s. Attack: %s' % ('att', args.attack))
    ...
    if args.attack == 'all':
        ...
        for attack in ['fgsm', 'bim-a', 'bim-b', 'jsma', 'cw-l2']:
            craft_one_type(sess, model, X_test, Y_test, 'att', attack,
                           args.batch_size)
    else:
        # Craft one specific attack type
        craft_one_type(sess, model, X_test, Y_test, 'att', args.attack, args.batch_size)
