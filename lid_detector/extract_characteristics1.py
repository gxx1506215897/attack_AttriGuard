...
BANDWIDTHS = {'mnist': 3.7926, 'cifar': 0.26, 'svhn': 1.00, 'att':1.00}
...
def input_adversarial_example(adversarial_example_filepath):
    ...
    input_adversarial_file=open(adversarial_example_filepath,'r')
    i=0
...
def main(args):
    ...
    model_file = os.path.join(PATH_DATA, "model_att.h5")
    ...
    for i in range(25):
        ...
        adv_file = os.path.join(PATH_DATA,'attriguard_adv',str(i)+'.txt')
        ...
        if args.attack == 'all':
            ...
            X_test_adv = input_adversarial_example(adv_file)
            ...
            noisy_file = os.path.join(PATH_DATA, 'Noisy_%s.npy' % (args.attack))
            if os.path.isfile(noisy_file):
                X_test_noisy = np.load(noisy_file)
            else:
                # Craft an equal number of noisy samples
                print('Crafting att noisy samples. ')
                X_test_noisy = get_noisy_samples(X_test, args.attack)
        ...
        if args.characteristic == 'kd':
            ...
            file_name = os.path.join('./data_grid_search/attriguard_adv_lid/', str(i)+'.npy')
            data = np.concatenate((characteristics, labels), axis=1)
            np.save(file_name, data)
...
if __name__ == "__main__":
    ...
    parser.set_defaults(dataset = 'att')
